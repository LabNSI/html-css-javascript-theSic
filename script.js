...
function resultat() {
  ...
  // ? Vérifier si le nom a été saisie. Si non, ajouter "\n - Nom" à message
  if(f.elements["nom"].value == "")  {
    message += "\n- Nom";
  }

  // ? Vérifier si une classe a été choisie. Si non, ajouter "\n - Classe" à message
  ...
  // ? Vérifier si les spécialités ont été choisie.
  for (var i=5; i < 12; i++)  {
  ...
  // ? Vérifier si 2 spécialités ont été choisie. Si non, ajouter "\n- Deux spécialités" à message
  if(cpt!=2){
    message += "\n- Deux spécialités";
  }

  // ? Vérifier si message ne contient pas "Compléter les champs :". Si non, mettre fin à l'exécution de la fonction
  ...
  // ? Vérifier si  une des 2 spécialités  est "NUMERIQUE_SC_INFORM". Si oui, ajouter "\nTu fais le bon choix !" à message. Si non, ajouter "\nEs-tu sur de ton choix ? !" à message
